...
namespace esphome
{
  namespace as7343
  {
    ...
    void AS7343Component::loop()
    {
      if (this->is_ready())
      {
        ...
        case State::NOT_INITIALIZED:
          // Add appropriate handling or a log message
          ESP_LOGW(TAG, "State: NOT_INITIALIZED. No actions taken.");
          break;

        case State::INITIAL_SETUP_COMPLETED:
        case State::COLLECTING_DATA_AUTO:
        case State::ADJUSTMENT_NEEDED:
        ...
        case State::READY_TO_PUBLISH_PART_3:
          ESP_LOGW(TAG, "State not implemented: %d", static_cast<int>(this->state_));
          break;
    ...
    void AS7343Component::calculate_and_publish()
    {
      ...
      if (this->par_ != nullptr)
      {
        this->par_->publish_state(par);
      }

      if (this->lux_ != nullptr)
      {
        this->lux_->publish_state(lux);
      }

      if (this->lux2_ != nullptr)
      {
        this->lux2_->publish_state(lux2);
      }
...
#define DIV64_U64(u64dividend, u64divisor) (u64dividend / u64divisor)
    ...
    bool AS7343Component::as7352_set_integration_time_us(uint32_t time_us)
    {
      ...
      ESP_LOGD(TAG, "for itime %lu : atime %u, astep %u, max_adc: %u", time_us, atime, astep, max_adc);
    ...
    T AS7343Component::get_highest_value(std::array<T, N> &data)
    {
      T max = 0;
